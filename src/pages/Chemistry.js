...
const Chemistry = ({ difficulty }) => {
  ...
  const show = () => {
    ...
    if (status) {
      opacity = parseFloat(
        window.getComputedStyle(status).getPropertyValue("opacity")
      );
      if (opacity < 1) {
        opacity = opacity + 0.1;
  ...
  return (
    <div className="main">
      ...
      <p className="status">
        {status
          ? status
          : "Find the elements listed in the right sidebar. Choose a spot in the periodic table to place each element."}
      </p>
        ...
        <div className="modal">
          <div className="form">
            ...
            <span className="message">
              Congratulations! You completed the round!
            </span>
            <input
              type="text"
              className="user-name"
